...
// SEO / meta / feed correctness against the relevant specs, plus status and
// canonical truth.
...
// ---- /index.html redirects to / (no duplicate home) ------------------------
...
// ---- unknown route is 404 with a non-reflecting canonical ------------------
...
test("unknown route: 404 + canonical/og:url point at home, not the path", async () => {
  ...
  assert.match(html, /<link rel="canonical" href="https:\/\/florasballet\.gr\/"/);
  assert.match(html, /<meta property="og:url" content="https:\/\/florasballet\.gr\/"/);
...
test("/nea/ serves 200 with canonical to /nea (no trailing slash)", async () => {
  const res = await request(base, "/nea/");
  assert.equal(res.status, 200);
  const html = res.body.toString("utf8");
  assert.match(html, /<link rel="canonical" href="https:\/\/florasballet\.gr\/nea"/);
});
...
test("200 routes have self-consistent canonical, og:url, title", async () => {
  ...
  for (const p of routes) {
    ...
    assert.match(html, /<meta property="og:locale" content="el_GR"/, `${p}: el_GR locale`);
  }
});
...
test("article meta matches article.js source", () => {
  ...
  const meta = server.computePageMeta(`/nea/${ARTICLE}`);
...
test("og:image dimensions are per-route accurate", async () => {
  // Home / interior pages share the 1200x630 social share image.
  ...
  // The article's og:image is its own cover (2000x1373), not the default.
  const article = (await request(base, `/nea/${ARTICLE}`)).body.toString("utf8");
  assert.match(article, /<meta property="og:image:width" content="2000" \/>/);
  assert.match(article, /<meta property="og:image:height" content="1373" \/>/);
  assert.ok(
...
// ---- JSON-LD Article: ISO dates, consistent author/publisher, el ----------

test("JSON-LD Article is schema-correct", async () => {
  const html = (await request(base, `/nea/${ARTICLE}`)).body.toString("utf8");
...
// ---- Home JSON-LD DanceSchool sameAs mirrors socialLinks -------------------

test("home JSON-LD DanceSchool sameAs is exactly site.config socialLinks", async () => {
  const html = (await request(base, "/")).body.toString("utf8");
  const block = html.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/)[1];
  const graph = JSON.parse(block)["@graph"];
  const school = graph.find((n) => {
    const t = n["@type"];
    return Array.isArray(t) ? t.includes("DanceSchool") : t === "DanceSchool";
  });
  assert.ok(school, "DanceSchool node present");
  assert.deepEqual(school.sameAs, SITE.socialLinks, "sameAs drifted from socialLinks");
  assert.equal(school.foundingDate, SITE.founded);
  assert.equal(school.address.streetAddress, SITE.address.street);
});

// ---- RSS 2.0 spec checks ----------------------------------------------------

test("rss.xml is RSS 2.0 (Greek), RFC-822 dates, guid and atom:link self", async () => {
  const xml = (await request(base, "/rss.xml")).body.toString("utf8");
  assert.match(xml, /^<\?xml version="1\.0" encoding="UTF-8"\?>/);
  assert.match(xml, /<rss version="2\.0"/);
  assert.match(xml, /<language>el<\/language>/);
  assert.match(xml, /<atom:link href="https:\/\/florasballet\.gr\/rss\.xml" rel="self"/);
  ...
  assert.match(xml, /<guid isPermaLink="true">https:\/\/florasballet\.gr\/nea\//);
});
...
test("article served HTML carries title/canonical/JSON-LD without JS", async () => {
  const html = (await request(base, `/nea/${ARTICLE}`)).body.toString("utf8");
  assert.ok(html.includes("Έναρξη εγγραφών"), "article title present in raw HTML");
  assert.match(html, new RegExp(`<link rel="canonical" href="${SITE.url}/nea/${ARTICLE}"`));
