...
// Accessibility: lock the WCAG 2.1 SC 1.4.3 contrast choices by computing the
...
test("white CTA text on the accent button meets WCAG AA", () => {
  // .btn-primary / .nav-cta render white text on --accent.
  const r = ratio("#ffffff", cssVar("--accent"));
  assert.ok(r >= 4.5, `white on --accent is ${r.toFixed(2)}:1 (needs 4.5:1)`);
});

test("cream footer text on the plum footer meets WCAG AA", () => {
  const r = ratio(cssVar("--cream"), cssVar("--ink"));
  assert.ok(r >= 4.5, `--cream on --ink is ${r.toFixed(2)}:1 (needs 4.5:1)`);
});

test("structural a11y guarantees", () => {
  assert.match(HTML, /<html lang="el">/, "lang attribute");
